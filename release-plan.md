...
1. **lockfile-hygiene** _(all triggers)_
   - Checkout
   - [`npm-lockfile-hygiene`](https://github.com/twilio/sdk-actions/blob/main/npm-lockfile-hygiene/action.yml) — fails if internal Artifactory hosts found in lockfile

2. **test** (Node matrix) — _needs: lockfile-hygiene_ _(all triggers)_
   - Checkout
