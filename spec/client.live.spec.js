...
describe('Live Twilio API smoke tests', function() {
    ...
    xit('makes a phone call using callbacks', function(done) {
        client.makeCall({
            to:config.to,
            from:config.from,
            url:voiceUrl
    ...
    xit('makes a phone call using promises', function(done) {
    ...
    xit('sends a text message using the callback interface', function(done) {
        client.sendMessage({
            to:config.to,
            from:config.from,
    ...
    xit('sends a text message using the promise interface', function(done) {
    ...
    xit('makes the two step phone number buying dance a bit easier', function(done) {
    ...
    xit('dies on the number buying process when an area code is not found', function(done) {
        client.availablePhoneNumbers('US').local.get({
            areaCode:'1337'
        }).then(function(searchResults) {
            if (searchResults.availablePhoneNumbers.length < 1) {
    ...
    xit('throws an auth error with a bad auth token (callbacks)', function(done) {
    ...
    xit('throws an auth error with a bad auth token (promises)', function(done) {
        var c = twilio(config.accountSid, 'foo bar');
    ...
    xit('throws an error properly when one call fails (promises)', function(done) {
        client.listMessages({
            from:config.from
        }).then(function(data) {
