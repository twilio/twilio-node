...
AccessToken.prototype.addGrant = function (resource, actions) {
    actions = [].concat(actions) || ACTION.ALL;
    this.grants.push({'res': resource, 'act': actions})
};

AccessToken.prototype.addEndPointGrant = function (endpoint, actions) {
    actions = [].contact(actions) || [ACTION.CLIENT.LISTEN, ACTION.CLIENT.INVITE];
    resource = 'sip' + endpoint + '@' + this.accountSid + 'endpoint.twilio.com';
    this.grants.push({'res': resource, 'act': actions})
};
...
AccessToken.prototype.toJwt = function (secret) {
    ...
    return jwtEncode(payload, secret, headers);
};
