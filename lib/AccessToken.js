 ...
 * @param {bool} [opts.staticJwt] - Should jwt be signed with time?
 */
...
_.extend(AccessToken.prototype, {
  ...
  toJwt: function(algorithm) {
    ...
    var payload = {
      jti: this.keySid + (this.staticJwt ? '-' + Math.floor(Date.now() / 1000) : ''),
      grants: grants
    };
    if (_.isNumber(this.nbf) && !this.staticJwt) { payload.nbf = this.nbf; }
