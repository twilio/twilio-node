...
function RestClient(sid, tkn, options) {
    ...
    this.sendSmsFeedback = function(messageSid, confirm, callback) {
        if (typeof(confirm) === 'function') {
            // confirm defaults to true if not specified - this is the only outcome that is necessary
            callback = confirm;
            confirm = true;
        }
        return this.accounts.messages(messageSid).feedback({outcome:confirm ? "confirmed" : "unconfirmed"}, callback)
    };
