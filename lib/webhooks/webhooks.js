 ...
 */
function validateRequest(authToken, twilioHeader, requestUrl, params) {
  twilioHeader = twilioHeader || '';
  const urlObject = new url.URL(requestUrl);
...
function validateRequestWithBody(authToken, twilioHeader, requestUrl, body) {
  const urlObject = new url.URL(requestUrl);
  return validateRequest(authToken, twilioHeader, requestUrl, {}) &&
