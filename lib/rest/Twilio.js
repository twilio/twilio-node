...
Twilio.prototype.request = function request(opts) {
  ...
  var [product, subdomain, domain] = uri.hostname.split('.');
  if (this.edge) {
    uri.hostname = ''.concat(product, '.', this.edge, '.', this.region || defaultRegion, '.', subdomain, '.', domain);
  } else if (this.region) {
    uri.hostname = ''.concat(product, '.', this.region, '.', subdomain, '.', domain);
  }
