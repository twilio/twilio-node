 ...
 *          Twilio region to use. Defaults to none or defaultRegion if edge provided
...
Twilio.prototype.request = function request(opts) {
  ...
  var prefix = uri.hostname.replace('.'.concat(domain), '');
  [product, edgeOrRegion, region] = prefix.split('.');
  if (edgeOrRegion && region) { // https://product.edge.region.subdomain.domain
    if (this.edge) {
      uri.hostname = [product, this.edge || edgeOrRegion, this.region || region, domain].join('.');
    } else if (this.region) {
      uri.hostname = [product, this.edge || edgeOrRegion, this.region, domain].join('.');
    }
  } else if (edgeOrRegion && !region) { // https://product.region.subdomain.domain
    if (this.edge) {
      uri.hostname = [product, this.edge || edgeOrRegion, this.region || region, domain].join('.');
    } else if (this.region) {
      uri.hostname = [product, this.region || edgeOrRegion, domain].join('.');
    }
  } else if (!edgeOrRegion && !region) { // https://product.subdomain.domain
    if (this.edge) {
      uri.hostname = [product, this.edge, this.region || defaultRegion, domain].join('.');
    } else if (this.region) {
      uri.hostname = [product, this.region, domain].join('.');
    }
  }
