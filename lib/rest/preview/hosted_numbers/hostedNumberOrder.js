...
HostedNumberOrderList = function HostedNumberOrderList(version) {
   ...
   * @param {string} opts.phoneNumber - An E164 formatted phone number.
   * @param {hosted_number_order.type} opts.type - Phone number type.
   * @param {string} opts.isoCountry - ISO country code.
   * @param {string} opts.smsCapability - Specify SMS capability to host.
  ...
  HostedNumberOrderListInstance.create = function create(opts, callback) {
    ...
    var data = values.of({
      ...
      'SmsCapability': _.get(opts, 'smsCapability'),
      'Email': _.get(opts, 'email'),
      'AccountSid': _.get(opts, 'accountSid'),
      'FriendlyName': _.get(opts, 'friendlyName'),
      'UniqueName': _.get(opts, 'uniqueName'),
      'CcEmails': _.get(opts, 'ccEmails'),
    ...
    promise = promise.then(function(payload) {
      deferred.resolve(new HostedNumberOrderInstance(
        this._version,
        payload,
        this._solution.sid
      ));
...
Object.defineProperty(HostedNumberOrderInstance.prototype,
  ...
  get: function() {
    if (!this._context) {
      this._context = new HostedNumberOrderContext(
        this._version,
        this._solution.sid
      );
    }
