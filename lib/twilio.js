...
IncomingPhoneNumber.prototype.setup = function(fn) {
    ...
    function configure() {
        var smsUrl = self.autoUri.addCallback('POST', handleRequest('incomingSms'));
        var callUrl = self.autoUri.addCallback('POST', handleRequest('incomingCall'));
        var statusUrl = self.autoUri.addCallback('POST', handleRequest('callStatus'));
        console.log(JSON.stringify(self.attrs));
        self.client.updateIncomingNumber(self.attrs.sid, 
            {
                VoiceUrl: callUrl,
                SmsUrl: smsUrl,
