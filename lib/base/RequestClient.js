 ...
 * @param {boolean} [opts.debug] - Show debug logs
 */
RequestClient.prototype.request = function(opts) {
  ...
  axios(options).then((response) => {
    if (opts.debug === true) {
      const requestLog = `Request: ${response.config.method.toUpperCase()} | ${response.config.url} | ${JSON.stringify(response.config.data)} | ${JSON.stringify(response.config.headers)}`
      const responseLog = `Response: ${response.status} | ${response.data.message} | ${response.data.more_info}`
